...
public class BankAccountApplicationService {
  ...
  @Autowired
  public BankAccountApplicationService(BankAccountDomainService bankAccountDomainService, BankAccountMapper bankAccountMapper, PagedResourcesAssembler<BankAccountResponseDTO> assembler) {
  ...
  public PagedModel<EntityModel<BankAccountResponseDTO>> getAccounts(Pageable pageable) {
    ...
    Link findAllLinks = linkTo(methodOn(BankAccountController.class).getBankAccounts(pageable.getPageNumber(), pageable.getPageSize(), String.valueOf(pageable.getSort()))).withSelfRel();
  ...
  public BankAccountResponseDTO updateAccount(UUID id, BankAccount account) {
    log.info("APPLICATION: Request received from controller and passing to the domain to update a bank account by id: {}", id);
    BankAccount updatedAccount = bankAccountDomainService.updateAccount(id, account);
