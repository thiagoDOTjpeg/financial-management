...
public interface JpaInvoiceRepository extends JpaRepository<Invoice, UUID> {
  ...
  @Query("SELECT i FROM Invoice i WHERE i.card.bankAccount.user.username = :username")
  Page<Invoice> findInvoiceByUsername(Pageable pageable, @Param("username") String username);
}
