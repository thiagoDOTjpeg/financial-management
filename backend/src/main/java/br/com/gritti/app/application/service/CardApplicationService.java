...
import br.com.gritti.app.shared.util.CardHateoasUtil;
...
public class CardApplicationService {
  ...
  public PagedModel<EntityModel<CardResponseDTO>> getCards(Pageable pageable, String username) {
    ...
    if(username != null && !username.isBlank()) {
      if(!isAdmin && !username.equals(currentUsername)){
        throw new IllegalArgumentException("Access denied, you don't have permission to access this resource");
      }
      cards = cardDomainService.getCards(pageable, username);
    } else if(!isAdmin){
    ...
    Page<CardResponseDTO> cardsWithLinks = cards.map(c -> {
      ...
      BankAccountHateoasUtil.addLinkGetMinimal(cardDto.getBankAccount());
      UserHateoasUtil.addLinkGetMinimalVO(cardDto.getBankAccount().getUser());
      return cardDto;
    });

    Link findAllLinks = linkTo(methodOn(CardController.class).getCards(pageable.getPageNumber(), pageable.getPageSize(), pageable.getSort().toString(), "")).withSelfRel();
    return assembler.toModel(cardsWithLinks, findAllLinks);
  ...
  public CardResponseDTO updateCard(UUID id, CardCreateDTO cardCreateDTO) {
